...
while True:
    ret, frame = vid.read()
    if recording:
        frame_filename = os.path.join(output_directory, f'frame_{frame_count:04d}.png') # Save the frame as an image
...
vid.release()   # Release video capture and writer objects
if out is not None:
    out.release()
cv2.destroyAllWindows() # Close all OpenCV windows

output_video_filename = 'output_video.avi'  # Code to convert saved frames to video
